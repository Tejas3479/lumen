...
Update user profile and preferences. (Also mounted at `PATCH /users/me/settings`).
...
Delete current user account and pseudonymise/soft-delete all related data (DPDP compliance).
...
Export all user data in JSON format for data portability (DPDP compliance).

| | |
|--|--|
| **Auth Required** | Yes |

**Response: 200 OK** — JSON containing profile, issues, comments, verifications, and points log.

---

### POST /users/me/fcm-token

Update current user's Firebase Cloud Messaging push token (also mounted at `POST /auth/me/fcm-token`).
...
Pre-submit semantic duplicate check. Called by `ReportIssueModal` before final submission.
...
```json
{
  ...
  "description": "Large pothole on road near Kalyani Nagar junction",
  "latitude": 12.9716,
  "longitude": 77.5946,
  "category_id": "uuid"
}
...
|-------|------|----------|-------------|
| `title` | string | ✓ | Min 5, max 256 characters |
| `description` | string | ✓ | Min 10, max 5000 characters |
| `latitude` | float | ✓ | GPS latitude |
| `longitude` | float | ✓ | GPS longitude |
| `category_id` | UUID string | — | Selected category ID |
...
### GET /ai/triage/{issue_id}

Retrieve the autonomous Triage Agent's detailed ReAct reasoning logs, recommended department, and recommended priority.
...
```json
{
  ...
  "reasoning_steps": [
    {
      "step": 1,
      "thought": "The user reported a severe water leakage. I should query departments to find the correct agency.",
      "tool_called": "get_department_recommendation",
      "tool_input": {"category": "water_leakage"},
      "tool_output": {"department": "BWSSB (Water Supply)"}
    }
  ],
...
Retrieve live status metrics, execution schedules, and run metrics for all 3 autonomous AI agents.
...
### GET /analytics/ward-report/{ward}

Retrieve the weekly narrative ward report generated by the Ward Report Agent for a specific ward.

| | |
|--|--|
| **Auth Required** | No |
| **Path Param** | `ward` — string (ward name) |
...
```json
{
  ...
  "narrative": "# Kalyani Nagar Weekly Ward Report\n\nDuring this past week, Kalyani Nagar experienced a significant increase in reported issues...",
  "stats": {
    "total_issues": 12,
    "resolved_issues": 8,
...
List all SLA breach escalations generated by the Escalation Agent.
...
[
  {
    "id": "uuid",
    "issue_id": "uuid",
    "escalated_at": "2026-06-29T10:30:00Z",
    "escalation_level": 1,
    "recipient_official_id": "uuid",
    "notes": "SLA breached for category drainage in Ward 12. Issue open for > 48 hours without action."
  }
]
```
...
Check the status of offline sync processes.
...
{
  "active_syncs": 0,
  "last_sync_at": "2026-06-29T11:00:00Z"
}
```
...
Lumen provides detailed metrics and connectivity check endpoints for monitoring.

### GET /health

Liveness probe for simple container running confirmation.
...
Readiness check to verify that both the database and redis connectivity are functional.
...
**Errors:** Returns `503 Service Unavailable` on database or redis disconnection.

---

### GET /health/metrics

Live operational statistics, active connection counts, and database pool stats.
